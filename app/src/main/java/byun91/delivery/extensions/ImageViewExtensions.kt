package byun91.delivery.extensions
...
fun ImageView.load(url: String, corner: Float = 0f, scaleType: Transformation<Bitmap> = CenterInside()) {
    Glide.with(this)
        .load(url)
        .transition(DrawableTransitionOptions.withCrossFade(factory)) // fade anim
        .diskCacheStrategy(DiskCacheStrategy.ALL) // cache 관리방식 -> 디스크
        .apply {
